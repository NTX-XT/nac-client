...
export class NWCTenant implements ILogging {
	...
	public async getWorkflow(workflowId: string): Promise<INWCWorkflowInfo> {
		return this.get(endpoints.WorkflowEndpointTemplate, { id: workflowId } as INWCWorkflowInfo).then(data => {
	...
	public async checkIfWorkflowExists(workflowName: string): Promise<boolean> {
		return this.get(endpoints.CheckWorkflowNameEndpointTemplate, { name: workflowName } as INWCWorkflowInfo)
			.then(data => {
