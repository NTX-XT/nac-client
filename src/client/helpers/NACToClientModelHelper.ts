...
export class NACToClientModelHelper {
  ...
  public static ConnectionSchema = (schema: connectionSchema): ConnectionSchema => ({
    ...
    properties: Object.assign(
      {},
      ...Object.keys(schema.properties).map<{ [key: string]: ConnectionProperty }>((key) => ({
        [key]: {
  ...
  public static WorkflowInstance = (instance: workflowInstance | startWorkflowResponse): WorkflowInstance => ({
    instanceId: (instance as startWorkflowResponse).id
      ? (instance as startWorkflowResponse).id
      : (instance as workflowInstance).instanceId,
    status: (instance as workflowInstance).status ?? 'Started',
  });
  ...
  public static WorkflowPermissions = (
    workflowOwners: permissionItem[],
    businessOwners: permissionItem[]
  ): Permission[] => {
  ...
  public static Workflow = (workflow: workflow, design: WorkflowDesign): Workflow => {
    ...
    for (const key of Object.keys(definition.inUseXtensions)) {
      if (
        typeof definition.inUseXtensions[key].xtension === 'string' ||
        definition.inUseXtensions[key].xtension instanceof String
      ) {
